...
def gameWinner(gameID, teamID, date):
    ...
    for i in data['games']:
        if i['gameId'] == gameID and 'endTimeUTC' in i:
            winner = i['hTeam']['teamId'] if int(i['hTeam']['score']) > int(
                i['vTeam']['score']) else i['vTeam']['teamId']
            loser = i['hTeam']['teamId'] if int(i['hTeam']['score']) < int(
                i['vTeam']['score']) else i['vTeam']['teamId']
            if winner == teamID:
            ...
            otherTeam = getTeam(
                i['hTeam']
                ['teamId']) if teamID != i['hTeam']['teamId'] else getTeam(
                    i['vTeam']['teamId'])
...
def printScores(date):
    ...
    for i in data['games']:
        ...
        if len(i['hTeam']['score']) > 0 and int(i['hTeam']['score']) > int(
                i['vTeam']['score']):
            home = "**" + home + "**"
        elif len(i['hTeam']['score']) > 0 and int(i['hTeam']['score']) < int(
                i['vTeam']['score']):
            away = "**" + away + "**"
        if (i['isGameActivated']):
            output += (str(count) + ") " + home + " " + i['hTeam']['score'] +
                       " vs " + away + " " + i['vTeam']['score'] + " Live\n")
        elif ('endTimeUTC' in i):

            output += (str(count) + ") " + home + " " + i['hTeam']['score'] +
                       " vs " + away + " " + i['vTeam']['score'] + " Final\n")
        else:
            output += (str(count) + ") " + home + " vs " + away + " @ " +
                       i['startTimeEastern'] + "\n")
        count += 1
    return output
...
def getEventId(date, gameId):
    URL = "http://data.nba.net/json/cms/noseason/game/" + date + "/" + str(
        gameId) + "/pbp_last.json"
    try:
        response = urlopen(URL)
    except URLError:
        return
...
def printBoxScore(date, gameId):
    ...
    for i in data['stats']['activePlayers']:
        if (i['min'] != '0:00' and i['min'] != '0' and i['min'] != ""):
            ...
            if i['teamId'] == homeId:
                h[("|" + name + " |" + minutes + " min|" + pts + " pts|" +
                   ast + " ast|" + reb + " reb|" + stl + " stl|" + blk +
                   " blk|" + to + " tov|")] = -int(pts)
            else:
                v[("|" + name + " |" + minutes + " min|" + pts + " pts|" +
                   ast + " ast|" + reb + " reb|" + stl + " stl|" + blk +
                   " blk|" + to + " tov|")] = -int(pts)
...
def printStandings():
    ...
    for i in data['league']['standard']['conference']['east']:
        ...
        if i['isWinStreak']:
            ...
            streak = ("-" + i['streak']).ljust(3)
        eastOut += (teamName.ljust(12) + "  | " + i['win'].ljust(2) + " | " +
                    i['loss'].ljust(2) + " |  " +
                    (i['lastTenWin'] + "-" + i['lastTenLoss']).ljust(4) +
                    "| " + streak + "|\n" +
                    "-------------------------------------\n")
    for i in data['league']['standard']['conference']['west']:
        ...
        if i['isWinStreak']:
            ...
            streak = ("-" + i['streak']).ljust(3)
        westOut += (teamName.ljust(12) + "  | " + i['win'].ljust(2) + " | " +
                    i['loss'].ljust(2) + " |  " +
                    (i['lastTenWin'] + "-" + i['lastTenLoss']).ljust(4) +
                    "| " + streak + "|\n\n" +
                    "-------------------------------------\n")
...
def addBet(userID, amount, gameID, betTeam, date):
    with open("bank.json") as file:
        ...
        if betTeam not in data[userID]['bets'][date][gameID]:
            data[userID]['bets'][date][gameID][betTeam] = {
                'amount': 0,
                'betComplete': 0
            }
...
def printBets(userId):
    ...
    listDates = [(start_day + datetime.timedelta(x)).strftime("%Y%m%d")
                 for x in range(14)]
    f = open('bank.json', "r")
    data = json.loads(f.read())
    ret = ""
                ...
                for teams in data[userId]['bets'][bd][games]:
                    try:
                        ...
                        if gameStatus[0] and gameStatus[1]:
                            ret += "-won $" + str(
                                data[userId]['bets'][bd][games][teams]
                                ['amount']) + " from " + gameStatus[
                                    2] + " vs " + gameStatus[3] + "\n"
            ...
            for dates in data[users]['bets']:
                for games in data[users]['bets'][dates]:
                    for teams in data[users]['bets'][dates][games]:
                        ...
                        if ret[0] and ret[1] and data[users]['bets'][dates][
                                games][teams]['betComplete'] == 0:
                            userFunds(
                                users, 2 * (data[users]['bets'][dates][games]
                                            [teams]['amount']))
                        if ret[0]:
                            data[users]['bets'][dates][games][teams][
                                'betComplete'] = 1
        json_dump(data, 'bank.json')
